(() => {
  ...
  const normalizePayload = (payload) => {
    // Eğer payload string ise (HTML sayfası olabilir), hata ver
    if (typeof payload === "string") {
      if (payload.includes("<!DOCTYPE html>") || payload.includes("ngrok")) {
        throw new Error("Ngrok browser warning sayfası algılandı. Lütfen ngrok URL'sine direkt tarayıcıdan gidip 'Visit Site' butonuna tıklayın.");
      }
      // String ama JSON değilse, JSON parse etmeyi dene
      try {
        payload = JSON.parse(payload);
      } catch (e) {
        throw new Error("API'den geçersiz yanıt formatı alındı: " + payload.substring(0, 100));
      }
    }
  ...
  const apiFetch = async (path, options = {}) => {
    try {
      ...
      // Ngrok browser warning sayfasını bypass et
      ...
      // Ngrok warning sayfası kontrolü
      if (typeof payload === "string" && payload.includes("ngrok") && payload.includes("<!DOCTYPE html>")) {
        console.error("[apiFetch] Ngrok warning sayfası algılandı!");
        throw new Error("Ngrok browser warning sayfası görüntülendi. Lütfen ngrok URL'sine direkt tarayıcıdan gidip 'Visit Site' butonuna tıklayın, sonra tekrar deneyin.");
      }
      ...
      // Ngrok warning sayfası kontrolü (HTML response) - header ekledik ama yine de kontrol edelim
      if (typeof payload === "string" && (payload.includes("ngrok") || payload.includes("<!DOCTYPE html>"))) {
        console.error("[apiFetch] Ngrok warning sayfası algılandı! (Header eklenmiş olmasına rağmen)");
        console.warn("[apiFetch] Not: ngrok-skip-browser-warning header'ı eklenmiş olmalı. Eğer hala bu hata görüyorsanız, ngrok'un free plan limitine takılmış olabilirsiniz.");
        const error = new Error("Ngrok browser warning sayfası algılandı. Lütfen ngrok URL'sine direkt tarayıcıdan gidip 'Visit Site' butonuna tıklayın, sonra tekrar deneyin.");
        error.status = 200; // Ngrok 200 döndürüyor ama HTML sayfası
        error.isNgrokWarning = true;
        throw error;
      }

      try {
        // Eğer payload string ise ve JSON değilse hata ver (ngrok HTML sayfası değilse)
        if (typeof payload === "string" && !payload.trim().startsWith("{") && !payload.trim().startsWith("[")) {
          // Eğer HTML değilse (ngrok kontrolü yukarıda yapıldı), başka bir hata olabilir
          console.error("[apiFetch] Payload string ama JSON değil:", payload.substring(0, 200));
          throw new Error("API'den beklenmeyen yanıt formatı alındı. Lütfen backend bağlantısını kontrol edin.");
        }
